...
export default function EndOfDayOverrun({ overrun, workingEnd }) {
  ...
  const headline =
    daysOver === 0
      ? `You stayed within your working hours every day this ${totalDays > 5 ? 'period' : 'week'}.`
      : `You went past ${workingEnd} on ${daysOver} of ${totalDays} days this ${totalDays > 5 ? 'period' : 'week'}.`
        ...
        {perDay.map((d) => (
          <div className="hbar-row" key={d.dateKey}>
            ...
            <span className="hbar-row__value">
              {d.overrunMinutes > 0 ? `+${formatDuration(d.overrunMinutes)}` : '—'}
            </span>
          </div>
        ))}
      </div>
